...
def create_model_and_optimizer(model_dict, optimizer_dict, device, state_sharding=True):
    ...
    if state_sharding:
        optimizer = OptimizerStateSharding(model.parameters(), optimizer_class.AdamW, lr=lr, weight_decay=weight_decay)
    else:
        optimizer = optimizer_class.AdamW(model.parameters(), lr=lr, weight_decay=weight_decay)
    return model, optimizer

def train(rank, world_size, nb_iters, model_dict, optimizer_dict, local_bs, nb_warmup=10, state_sharding=True):
